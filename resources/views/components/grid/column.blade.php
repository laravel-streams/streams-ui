    ...
    {!!
        $attributes
            ->class([
                ...
                // 'col-span-' . $default => $default && (! $hidden),
                // 'sm:col-span-' . $sm => $sm && (! $hidden),
                // 'md:col-span-' . $md => $md && (! $hidden),
                // 'lg:col-span-' . $lg => $lg && (! $hidden),
                // 'xl:col-span-' . $xl => $xl && (! $hidden),
                // '2xl:col-span-' . $twoXl => $twoXl && (! $hidden),
                // 'col-start-' . $defaultStart => $defaultStart && (! $hidden),
                // 'sm:col-start-' . $smStart => $smStart && (! $hidden),
                // 'md:col-start-' . $mdStart => $mdStart && (! $hidden),
                // 'lg:col-start-' . $lgStart => $lgStart && (! $hidden),
                // 'xl:col-start-' . $xlStart => $xlStart && (! $hidden),
                // '2xl:col-start-' . $twoXlStart => $twoXlStart && (! $hidden),
                'hidden' => $hidden || $default === 'hidden',
