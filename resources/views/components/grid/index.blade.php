    ...
    {{
        $attributes
            ->class([
                ...
                // 'grid-cols-' . $default => $default && ($direction === 'row'),
                // 'columns-' . $default => $default && ($direction === 'column'),
                // 'sm:grid-cols-' . $sm => $sm && ($direction === 'row'),
                // 'sm:columns-' . $sm => $sm && ($direction === 'column'),
                // 'md:grid-cols-' . $md => $md && ($direction === 'row'),
                // 'md:columns-' . $md => $md && ($direction === 'column'),
                // 'lg:grid-cols-' . $lg => $lg && ($direction === 'row'),
                // 'lg:columns-' . $lg => $lg && ($direction === 'column'),
                // 'xl:grid-cols-' . $xl => $xl && ($direction === 'row'),
                // 'xl:columns-' . $xl => $xl && ($direction === 'column'),
                // '2xl:grid-cols-' . $twoXl => $twoXl && ($direction === 'row'),
                // '2xl:columns-' . $twoXl => $twoXl && ($direction === 'column'),
            ])
            ->style(
                match ($direction) {
                    'column' => [
